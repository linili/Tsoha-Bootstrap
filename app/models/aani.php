...
class Aani extends BaseModel {
    ...
    public function validate_aani() {
        $errors = array();
        $query = DB::connection()->prepare('SELECT * FROM Aani WHERE aanestys_id = :aanestys_id AND aanestaja_id = :aanestaja_id');
        $query->execute(array('aanestys_id' => $this->aanestys_id, 'aanestaja_id' => $this->aanestaja_id));
        $row = $query->fetch();

        if ($row) {
            $errors[] = 'Olet jo äänestänyt!';
        }
        return $errors;
    }
